...
La mente baja/sube confianza según historial real (papel + dinero ponderado).
"""
...
# Compat: tests y código legacy
SENALES = SENALES_MENTE
...
def senales_de_pred(pred: dict, juego: dict | None = None, cfg: dict | None = None) -> list[str]:
    """Extrae señales activas del pick (alertas mente / contexto)."""
    return extraer_senales_aprendizaje(pred, juego, cfg)


def senales_de_juego(juego: dict, cfg: dict | None = None) -> list[str]:
    """Señales para decisión en vivo (bloqueo T-60)."""
    return extraer_senales_aprendizaje(juego, juego, cfg)


def _inc(cont: dict, key: str, delta: float) -> None:
    cont[key] = float(cont.get(key) or 0) + float(delta)
...
def _recalcular_penalizaciones(stats: dict) -> None:
    ...
    for senal, p in (stats.get("patrones") or {}).items():
        if not isinstance(p, dict):
            continue
        n_pasar = float(p.get("pasar_ok") or 0) + float(p.get("pasar_miss") or 0)
        n_ap = float(p.get("apostar_ok") or 0) + float(p.get("apostar_fail") or 0)
        score = 0
        if n_pasar >= 4:
            miss = float(p.get("pasar_miss") or 0) / n_pasar
            ok = float(p.get("pasar_ok") or 0) / n_pasar
            if miss >= 0.55:
                score -= 1
            if miss >= 0.7:
                score -= 1
            if ok >= 0.6:
                score += 1
        if n_ap >= 4:
            fail = float(p.get("apostar_fail") or 0) / n_ap
            if fail >= 0.55:
                score -= 1
            if fail >= 0.7:
                score -= 1
            if float(p.get("apostar_ok") or 0) / n_ap >= 0.6:
                score += 1
...
def actualizar_stats_tras_liquidar(
    ...
    senales = senales_de_pred(pred, juego, cfg)
    for s in senales:
        p = stats["patrones"].setdefault(
            s,
            {"n": 0.0, "pasar_ok": 0.0, "pasar_miss": 0.0, "apostar_ok": 0.0, "apostar_fail": 0.0},
        )
        _inc(p, "n", peso)
        _inc(p, outcome, peso)
    ...
    print(
        f"[MENTE-APRENDIZAJE] {decision} → {resultado} peso={peso} señales={senales} "
        f"pen={ {k: stats['penalizacion'].get(k) for k in senales} }"
    )
    return {"decision": decision, "resultado": resultado, "senales": senales, "peso": peso}
...
def aplicar_aprendizaje_a_conclusion(
    ...
    if decision == "APOSTAR" and pen > 0:
        ...
        conclusion["razones"] = list(conclusion.get("razones") or []) + [
            f"Aprendizaje: señales reforzadas (+{pen})"
        ]
    ...
    if decision == "PASAR" and pen <= -1 and fuente in ("heuristica", "groq"):
        ...
        if memoria:
            ...
            for s in senales:
                ...
                n = float(p.get("pasar_ok") or 0) + float(p.get("pasar_miss") or 0)
                if n >= 4 and float(p.get("pasar_miss") or 0) / n >= 0.55:
...
def texto_aprendizaje_para_prompt(memoria: dict | None, max_n: int = 8) -> str:
    if not memoria:
    ...
    for s, v in sorted(pen.items(), key=lambda kv: abs(int(kv[1] or 0)), reverse=True):
        ...
        lineas.append(
            f"- {s}: penalización {v} "
            f"(pasar_ok={p.get('pasar_ok', 0):.0f} miss={p.get('pasar_miss', 0):.0f} "
            f"ap_ok={p.get('apostar_ok', 0):.0f} ap_fail={p.get('apostar_fail', 0):.0f})"
        )
        if len(lineas) >= max_n:
            break
...
def recomputar_stats_desde_historial(memoria: dict, max_dias: int = 60, cfg: dict | None = None) -> int:
    stats = asegurar_mente_stats(memoria)
    stats["decisiones"] = {
        "APOSTAR": {"n": 0, "aciertos": 0, "fallos": 0},
        "PASAR": {"n": 0, "evito_fallo": 0, "oportunidad_perdida": 0},
    ...
    stats["patrones"] = {
        s: {"n": 0.0, "pasar_ok": 0.0, "pasar_miss": 0.0, "apostar_ok": 0.0, "apostar_fail": 0.0}
        for s in SENALES_MENTE
    }
    stats["vistos"] = []
    n = 0
    ...
    for dia in dias:
        for pred in dia.get("predicciones") or []:
            if not isinstance(pred, dict):
                continue
            if actualizar_stats_tras_liquidar(memoria, pred, cfg=cfg):
                n += 1
    return n
